...
# Constantes
ESTADOS_MAT = ["disponible", "reservado", "mantenimiento"]
...
registro = {}
prestamos = {}
entorno = {"ubic": {"tipo": "almacen", "zona": None, "disciplina": None}, "user": None}
seq_mat = 1
seq_usr = 1001
...
def convertir_fecha(cadena):
    try:
        return datetime.strptime(cadena.strip(), "%Y-%m-%d").date()
    except:
        return None

def siguiente_id_mat():
    global seq_mat
    mid = seq_mat
    seq_mat += 1
    return mid

def siguiente_id_usr():
    global seq_usr
    uid = seq_usr
    seq_usr += 1
    return uid
...
def elegir(msj, opciones):
    lower = [v.lower() for v in opciones]
    while True:
        s = input(msj).strip()
        if s.lower() in lower:
            return opciones[lower.index(s.lower())]
...
def crear_material(nombre, marca, fecha, estado, origen, ubic):
    mid = siguiente_id_mat()
    inventario[mid] = {
        "id": mid,
        "nombre": nombre,
        "marca": marca,
...
def inicializar_datos():
    base_fecha = fecha_hoy() - timedelta(days=60)
    marcas = ["ProSport", "AquaMax", "CourtX", "PowerFit", "UrbanGym"]
    nombres_base = ["Pelota basket", "Pelota tenis", "Raqueta tenis", "Pala padel",
                    "Gafas natacion", "Aletas", "Mancuernas 5kg", "Esterilla", "Balon 3kg",
                    "Comba", "Casco boxeo", "Guantes boxeo", "Conos", "Chaleco lastre", "Bandas"]
    for i, nom in enumerate(nombres_base):
        crear_material(nom, marcas[i % len(marcas)], base_fecha + timedelta(days=i),
                       "disponible", None, {"tipo": "almacen", "zona": None, "disciplina": None})
    for zona in ZONAS:
        propios = [("Kit piscina", "AquaMax"), ("Carro pelotas", "ProSport"), ("Red tenis", "CourtX"),
                   ("Red padel", "CourtX"), ("Banco press", "PowerFit"), ("Cinta correr", "UrbanGym"),
                   ("Eliptica", "UrbanGym"), ("Colchoneta", "ProSport"), ("Balon pro", "ProSport"),
                   ("Jabalina", "ProSport")]
        for i, (nom, marc) in enumerate(propios):
            crear_material(nom, marc, base_fecha + timedelta(days=10 + i),
                           "disponible", zona, {"tipo": "centro", "zona": zona, "disciplina": None})
        otros = [x for x in ZONAS if x != zona]
        prest = [("Vallas", "ProSport"), ("Porteria sala", "ProSport"), ("Cronometro", "UrbanGym"),
                 ("Balon 5kg", "PowerFit"), ("Saco boxeo", "PowerFit")]
        for i, (nom, marc) in enumerate(prest):
            crear_material(nom, marc, base_fecha + timedelta(days=30 + i),
                           "disponible", otros[i % 3], {"tipo": "centro", "zona": zona, "disciplina": None})
        por_deporte = {
        ...
        for act in disciplinas[zona]:
            lista = por_deporte.get(act, [("Gen A", "ProSport"), ("Gen B", "PowerFit"), ("Gen C", "UrbanGym")])
            for j, (nom, marc) in enumerate(lista[:3]):
                crear_material(nom, marc, base_fecha + timedelta(days=50 + j),
                               "disponible", zona, {"tipo": "actividad", "zona": zona, "disciplina": act})
